...
At the start of the encounter phase each year, the system SHALL snapshot `Nk` (cultivator count per level) and `N` (total cultivator count, Lv >= 1). These values remain fixed for the entire encounter phase. For each Lv=k cultivator, an encounter triggers with probability `Nk / N`. Lv0 cultivators SHALL NOT participate.

#### Scenario: Snapshot-based encounter probability
- **WHEN** the encounter phase begins with 8000 Lv1 cultivators and 10000 total
