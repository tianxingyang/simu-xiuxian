...
系统 SHALL 定义 `LevelStat` 接口，包含四个 number 字段：`ageAvg`、`ageMedian`、`courageAvg`、`courageMedian`。所有数值 SHALL 在引擎 `getSummary()` 计算完成后立即通过 `round1()` 保留一位小数存入 `LevelStat`，下游无需再处理精度。

#### Scenario: LevelStat field completeness
- **WHEN** 系统生成某个境界的统计数据
- **THEN** 该 `LevelStat` SHALL 包含 `ageAvg`、`ageMedian`、`courageAvg`、`courageMedian` 四个字段，且均已 round1 处理
...
### PBT: Bounds
对非空境界 i，`ageAvg` 和 `ageMedian` SHALL 在该境界实际 age 值的 `[min, max]` 范围内（round1 容差）。`courageAvg` 和 `courageMedian` 同理，且 SHALL 在 `[0, 1]` 内。
