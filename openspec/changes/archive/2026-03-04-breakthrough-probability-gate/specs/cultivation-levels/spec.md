...
Lv0→Lv1 升级 SHALL 在修为达到 `threshold(1)` 时自动触发，不经过突破概率门。

Lv1+ 升级（Lv1→Lv2, Lv2→Lv3, ..., Lv6→Lv7）SHALL 在修为达到 `threshold(level+1)` 后，通过突破概率门判定。仅当突破成功时才执行升级。每个路径（`tickCultivators` / `resolveCombat`）内每个修仙者最多允许一次突破尝试，两路径独立计算。
...
#### Scenario: Lv0 auto-promotes to Lv1
- **WHEN** Lv0 修仙者 cultivation 达到 10
- **THEN** SHALL 自动升级至 Lv1，不经过突破概率门
