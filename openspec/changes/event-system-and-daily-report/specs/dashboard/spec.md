...
- **THEN** SHALL 向所有客户端广播一条 `{ type: 'tick', tickId: N, summaries: [...], events: [...] }` 消息，然后等待 ack

#### Scenario: ACK message type
- **WHEN** 前端处理完 tick 消息
- **THEN** SHALL 发送 `{ type: 'ack', tickId: N }` 到 WebSocket server（N 为最近处理的 tick 的 tickId）
...
- **WHEN** rAF 回调 flush 完所有缓冲消息且至少包含一条 tick 消息
- **THEN** SHALL 向 WebSocket server 发送恰好一个 `{ type: 'ack', tickId }` （tickId 取 flush 中最大的 tickId）

#### Scenario: Flush without tick skips ACK
- **WHEN** rAF 回调 flush 缓冲消息但仅含 paused 消息（无 tick）
- **THEN** SHALL 不发送 ack
...
### Property: Reconnect baseline sync
每次（重）连接后，UI 收到 `state` 快照即可重建正确的仪表盘基线。
- **Falsification**: 随机重连时机和消息流，schema 校验所有包，验证重连后基线同步。

### Property: Reconnect backoff schedule
重连延迟序列为单调递增且有上界：`1,2,4,8,...,30,30,...` 秒；`connectionStatus ∈ {'connected','connecting','disconnected'}`。
- **Falsification**: 模拟连续断连失败 + mount/unmount 抖动，断言延迟调度和状态枚举。

### Property: rAF render at-most-once per frame
每帧最多触发一次 `setState`；`state/reset-done` 绕过缓冲区立即处理。flush 含 tick 时恰好发一个 ack（携带最大 tickId），不含 tick 则不发 ack。
- **Falsification**: 注入突发混合消息（对齐/不对齐帧边界），断言渲染/ack 计数。

### Property: Level table always 8 rows
统计表格 SHALL 固定 8 行（Lv0-Lv7）；空境界展示 `-`；非空境界值 = 最新 `YearSummary`。
- **Falsification**: 随机稀疏/缺失境界的 summary + 快速更新，断言行基数和单元格规则。
