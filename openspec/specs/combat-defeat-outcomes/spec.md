...
每个修士每年 SHALL 最多承受一次战败结局。所有战败结局（包括轻伤和经脉受损）SHALL 触发同年战败锁定。被锁定的修士在本年剩余时间内：

- SHALL NOT 主动发起遭遇
- SHALL NOT 被选为对手
- SHALL NOT 计入遭遇概率快照 Nk/N
- 存活败者 SHALL 立即从 `levelArrayCache` 中移除
- 存活败者 SHALL 被记录在本轮败者 Set 中
- 败者 Set SHALL 在下一年重置
...
5. 存活结局掷骰：`prng()` 选择轻伤/重伤/损失修为/经脉受损/跌境（仅死亡判定未命中时）
